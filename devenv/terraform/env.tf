...
module "portal" {
  ...
  cd = {
    name      = "portal",
    command   = ["--bootstrap","--conf=/conf/portal.conf"],
    port      = 3001,
    log_group = aws_cloudwatch_log_group.env.name,
    image     = join(":", [data.terraform_remote_state.base.outputs.portal["ecr"], var.portal])
    mounts = [
      { src = "config", dest = "/conf" }
    ],
    env = [],
    secrets = [
      { name = "PORTAL_LICENSEKEY", from = local.dash_license }
    ],
    region = data.terraform_remote_state.base.outputs.region
  }
}