...
exec > >(tee -a "$LOG_DIR/sync-remaining.log") 2>&1

echo "Remaining migration sync started at $(date)"
...
rsync_dir() {
  ...
  echo " -> $dst/"
  mkdir -p "$dst"
  rsync -aEH --partial --exclude-from "$EXCLUDES" "$src/" "$dst/"
}
...
echo
echo "Remaining migration sync finished at $(date)"
